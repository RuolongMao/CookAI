...
class DashboardCRUD:
    ...
    def get_personal_recipes(self, db: Session, user_name: str) -> List[models.Recipes]:
        return db.query(models.Recipes).filter(models.Recipes.user_name == user_name).all()
    ...
    def filter_recipes(self, db: Session, est_time_min: Optional[int] = None, est_time_max: Optional[int] = None, est_cost_min: Optional[float] = None, est_cost_max: Optional[float] = None, calories_min: Optional[int] = None, calories_max: Optional[int] = None,tastes: Optional[List[str]] = None):
        ...
        if est_cost_min is not None:
            query = query.filter(models.Recipes.est_cost >= est_cost_min)
        if est_cost_max is not None:
            query = query.filter(models.Recipes.est_cost <= est_cost_max)
